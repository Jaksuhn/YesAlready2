...
public static unsafe class EventController
{
    ...
    public class EventSubscriber
    {
        ...
        private static bool IsAddonPointer(ParameterInfo p)
        {
        ...
        private bool DetermineInvokeKind()
        {
            ...
            {
                Svc.Log.Error($"Exeception during {nameof(DetermineInvokeKind)}", ex);
            }

            Svc.Log.Error($"[{nameof(EventController)}] Failed to determine a valid delegate type for '{Feature.GetType().Name}.{Method.Name}'");

            foreach (var param in p)
            {
                Svc.Log.Error($"[{nameof(EventController)}] \t - {param.ParameterType} {param.Name}");
            }
    ...
    public static void RegisterEvents(BaseFeature feature)
    {
        ...
        foreach (var method in methods)
        {
            ...
            foreach (var attr in method.GetCustomAttributes<AddonEventAttribute>())
            {
                ...
                {
                    Svc.Log.Verbose($"[{nameof(EventController)}] {feature.Name} requesting event '{attr.Event}' on method '{method.Name}' for addon '{addon}'");
